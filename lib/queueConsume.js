...
module.exports.consumeFromQueue = function consumeFromQueue(id, queueName, next) {
  try {
    inputHelper.checkConsumeInput(id, queueName);
    inputHelper.checkInOutQueueName(queueName);
          ...
          channel.consume(queueName, (msg) => {
            if (queueName.includes('input-')) {
              next(
                ...
                channel,
                msg,
                queueHelper.createDefaultServiceTimeout(msg.properties.correlationId),
              );
            } else if (queueHelper.findId(msg) === id) {
              next(
                null,
                channel,
                msg,
